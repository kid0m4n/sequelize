...
module.exports = (function() {
  var DAO = function(values, options, isNewRecord) {
    ...
    if (this.integerValues.length) {
      this.integerValues.forEach(function (name) {
        if(self && ("string" === typeof self[name])) {
          self[name] = parseInt(self[name])
        }
      });
    }

    if (this.floatValues.length) {
      this.floatValues.forEach(function (name) {
        if(self && ("string" === typeof self[name])) {
          self[name] = parseFloat(self[name])
        }
      });
    }
  }
